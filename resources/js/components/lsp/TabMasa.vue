<script setup>
import { ref, onMounted, watch } from "vue";
import { periodeService, masaService } from "../../services/lspService";
import BaseModal from "../BaseModal.vue";

const emit = defineEmits(["toast"]);

const periodeList = ref([]);
const selectedPeriode = ref("");
const list = ref([]);
const loading = ref(false);
const modal = ref(false);
const isEdit = ref(false);
const saving = ref(false);
const error = ref("");
const editId = ref(null);
const form = ref({ tanggal_mulai: "", tanggal_selesai: "" });
const toggling = ref(null);

const toggle = async (item) => {
    toggling.value = item.kdlsp_periode_masa;
    try {
        await masaService.toggle(
            selectedPeriode.value,
            item.kdlsp_periode_masa,
        );
        emit("toast", {
            message: `Masa periode ${item.isActive ? "dinonaktifkan" : "diaktifkan"}`,
        });
        fetchMasa();
    } catch {
        emit("toast", { message: "Gagal mengubah status", type: "error" });
    } finally {
        toggling.value = null;
    }
};

const fetchPeriode = async () => {
    const res = await periodeService.getAll();
    periodeList.value = res.data;
    if (res.data.length > 0) selectedPeriode.value = res.data[0].kdlsp_periode;
};

const fetchMasa = async () => {
    if (!selectedPeriode.value) return;
    loading.value = true;
    try {
        const res = await masaService.getAll(selectedPeriode.value);
        list.value = res.data;
    } catch {
        emit("toast", { message: "Gagal memuat masa periode", type: "error" });
    } finally {
        loading.value = false;
    }
};

watch(selectedPeriode, fetchMasa);

const openCreate = () => {
    if (!selectedPeriode.value) {
        emit("toast", {
            message: "Pilih periode terlebih dahulu",
            type: "error",
        });
        return;
    }
    form.value = { tanggal_mulai: "", tanggal_selesai: "" };
    isEdit.value = false;
    editId.value = null;
    error.value = "";
    modal.value = true;
};

const openEdit = (item) => {
    form.value = {
        tanggal_mulai: item.tanggal_mulai,
        tanggal_selesai: item.tanggal_selesai,
    };
    isEdit.value = true;
    editId.value = item.kdlsp_periode_masa;
    error.value = "";
    modal.value = true;
};

const submit = async () => {
    error.value = "";
    saving.value = true;
    try {
        if (isEdit.value) {
            await masaService.update(
                selectedPeriode.value,
                editId.value,
                form.value,
            );
            emit("toast", { message: "Masa periode berhasil diperbarui" });
        } else {
            await masaService.create(selectedPeriode.value, form.value);
            emit("toast", { message: "Masa periode berhasil ditambahkan" });
        }
        modal.value = false;
        fetchMasa();
    } catch (err) {
        error.value = err.response?.data?.message || "Terjadi kesalahan";
    } finally {
        saving.value = false;
    }
};

const destroy = async (item) => {
    if (!confirm("Hapus masa periode ini?")) return;
    try {
        await masaService.destroy(
            selectedPeriode.value,
            item.kdlsp_periode_masa,
        );
        emit("toast", { message: "Masa periode berhasil dihapus" });
        fetchMasa();
    } catch {
        emit("toast", {
            message: "Gagal menghapus masa periode",
            type: "error",
        });
    }
};

const formatDate = (d) =>
    d
        ? new Date(d).toLocaleDateString("id-ID", {
              day: "2-digit",
              month: "long",
              year: "numeric",
          })
        : "-";

onMounted(fetchPeriode);
</script>

<template>
    <div>
        <!-- Toolbar -->
        <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
            <div>
                <h3 class="text-sm font-semibold text-[#1e3329]">
                    Masa Periode
                </h3>
                <p class="text-xs text-[#7aab95] mt-0.5">
                    Atur rentang tanggal tiap periode
                </p>
            </div>
            <div class="flex items-center gap-3">
                <select
                    v-model="selectedPeriode"
                    class="border border-[#c8ddd6] rounded-lg px-3 py-2 text-sm text-[#1e3329] focus:outline-none focus:border-[#4a7c6b] bg-white"
                >
                    <option value="" disabled>Pilih Periode</option>
                    <option
                        v-for="p in periodeList"
                        :key="p.kdlsp_periode"
                        :value="p.kdlsp_periode"
                    >
                        {{ p.periode }}
                    </option>
                </select>
                <button
                    @click="openCreate"
                    class="flex items-center gap-2 bg-[#2d4a3e] hover:bg-[#3d6355] text-white text-xs font-medium px-4 py-2 rounded-lg transition-all"
                >
                    <svg
                        class="w-3.5 h-3.5"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M12 4v16m8-8H4"
                        />
                    </svg>
                    Tambah Masa
                </button>
            </div>
        </div>

        <!-- Table -->
        <div
            class="bg-white rounded-xl border border-[#dde8e3] overflow-hidden"
        >
            <div v-if="loading" class="flex items-center justify-center py-16">
                <svg
                    class="w-5 h-5 animate-spin text-[#4a7c6b]"
                    fill="none"
                    viewBox="0 0 24 24"
                >
                    <circle
                        class="opacity-25"
                        cx="12"
                        cy="12"
                        r="10"
                        stroke="currentColor"
                        stroke-width="4"
                    ></circle>
                    <path
                        class="opacity-75"
                        fill="currentColor"
                        d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"
                    ></path>
                </svg>
            </div>
            <table v-else class="w-full text-sm">
                <thead>
                    <tr class="bg-[#f0f4f1] border-b border-[#dde8e3]">
                        <th
                            class="text-left px-5 py-3 text-xs font-semibold text-[#3d6355] uppercase tracking-wider w-12"
                        >
                            No
                        </th>
                        <th
                            class="text-left px-5 py-3 text-xs font-semibold text-[#3d6355] uppercase tracking-wider"
                        >
                            Tanggal Mulai
                        </th>
                        <th
                            class="text-left px-5 py-3 text-xs font-semibold text-[#3d6355] uppercase tracking-wider"
                        >
                            Tanggal Selesai
                        </th>
                        <th
                            class="text-left px-5 py-3 text-xs font-semibold text-[#3d6355] uppercase tracking-wider"
                        >
                            Status
                        </th>
                        <th
                            class="text-right px-5 py-3 text-xs font-semibold text-[#3d6355] uppercase tracking-wider"
                        >
                            Aksi
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="!selectedPeriode">
                        <td
                            colspan="4"
                            class="text-center py-12 text-[#7aab95] text-sm"
                        >
                            Pilih periode untuk melihat masa
                        </td>
                    </tr>
                    <tr v-else-if="list.length === 0">
                        <td
                            colspan="4"
                            class="text-center py-12 text-[#7aab95] text-sm"
                        >
                            Belum ada masa periode
                        </td>
                    </tr>
                    <tr
                        v-for="(item, i) in list"
                        :key="item.kdlsp_periode_masa"
                        class="border-b border-[#f0f4f1] hover:bg-[#f9fbfa] transition-colors"
                    >
                        <td class="px-5 py-3.5 text-slate-400 text-xs">
                            {{ i + 1 }}
                        </td>
                        <td class="px-5 py-3.5 font-medium text-[#1e3329]">
                            {{ formatDate(item.tanggal_mulai) }}
                        </td>
                        <td class="px-5 py-3.5 text-slate-500">
                            {{ formatDate(item.tanggal_selesai) }}
                        </td>
                        <td class="px-5 py-3.5">
                            <button
                                @click="toggle(item)"
                                :disabled="toggling === item.kdlsp_periode_masa"
                                class="flex items-center gap-2.5 group"
                                :title="
                                    item.isActive
                                        ? 'Klik untuk nonaktifkan'
                                        : 'Klik untuk aktifkan'
                                "
                            >
                                <div
                                    :class="
                                        item.isActive
                                            ? 'bg-[#2d4a3e]'
                                            : 'bg-slate-200'
                                    "
                                    class="relative w-10 h-5 rounded-full transition-colors duration-300 shrink-0"
                                >
                                    <div
                                        :class="
                                            item.isActive
                                                ? 'translate-x-5'
                                                : 'translate-x-0.5'
                                        "
                                        class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-300"
                                    ></div>
                                    <div
                                        v-if="
                                            toggling === item.kdlsp_periode_masa
                                        "
                                        class="absolute inset-0 flex items-center justify-center rounded-full bg-black/10"
                                    >
                                        <svg
                                            class="w-3 h-3 animate-spin text-white"
                                            fill="none"
                                            viewBox="0 0 24 24"
                                        >
                                            <circle
                                                class="opacity-25"
                                                cx="12"
                                                cy="12"
                                                r="10"
                                                stroke="currentColor"
                                                stroke-width="4"
                                            ></circle>
                                            <path
                                                class="opacity-75"
                                                fill="currentColor"
                                                d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"
                                            ></path>
                                        </svg>
                                    </div>
                                </div>
                                <span
                                    :class="
                                        item.isActive
                                            ? 'text-[#2d4a3e] font-semibold'
                                            : 'text-slate-400'
                                    "
                                    class="text-xs transition-colors duration-200 select-none"
                                >
                                    {{ item.isActive ? "Aktif" : "Nonaktif" }}
                                </span>
                            </button>
                        </td>
                        <td class="px-5 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    @click="openEdit(item)"
                                    class="px-3 py-1.5 text-xs text-[#4a7c6b] bg-[#eaf2ee] hover:bg-[#d4e8dd] rounded-lg transition-all font-medium"
                                >
                                    Edit
                                </button>
                                <button
                                    @click="destroy(item)"
                                    class="px-3 py-1.5 text-xs text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-all font-medium"
                                >
                                    Hapus
                                </button>
                            </div>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>

        <!-- Modal -->
        <BaseModal
            :show="modal"
            :title="isEdit ? 'Edit Masa Periode' : 'Tambah Masa Periode'"
            @close="modal = false"
        >
            <div class="space-y-4">
                <div>
                    <label
                        class="text-xs font-semibold text-[#3d6355] uppercase tracking-wider block mb-1.5"
                        >Tanggal Mulai</label
                    >
                    <input
                        v-model="form.tanggal_mulai"
                        type="date"
                        class="w-full border border-[#c8ddd6] rounded-xl px-4 py-2.5 text-sm text-[#1e3329] focus:outline-none focus:border-[#4a7c6b] focus:ring-2 focus:ring-[#4a7c6b]/15 transition-all"
                    />
                </div>
                <div>
                    <label
                        class="text-xs font-semibold text-[#3d6355] uppercase tracking-wider block mb-1.5"
                        >Tanggal Selesai</label
                    >
                    <input
                        v-model="form.tanggal_selesai"
                        type="date"
                        class="w-full border border-[#c8ddd6] rounded-xl px-4 py-2.5 text-sm text-[#1e3329] focus:outline-none focus:border-[#4a7c6b] focus:ring-2 focus:ring-[#4a7c6b]/15 transition-all"
                    />
                </div>

                <div
                    v-if="error"
                    class="flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 rounded-xl px-4 py-3 text-xs"
                >
                    <svg
                        class="w-3.5 h-3.5 shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"
                        />
                    </svg>
                    {{ error }}
                </div>

                <div class="flex justify-end gap-2 pt-1">
                    <button
                        @click="modal = false"
                        class="px-4 py-2 text-xs font-medium text-slate-500 hover:bg-slate-100 rounded-lg transition-all"
                    >
                        Batal
                    </button>
                    <button
                        @click="submit"
                        :disabled="
                            saving ||
                            !form.tanggal_mulai ||
                            !form.tanggal_selesai
                        "
                        class="px-5 py-2 text-xs font-semibold bg-[#2d4a3e] hover:bg-[#3d6355] disabled:opacity-50 text-white rounded-lg transition-all flex items-center gap-2"
                    >
                        <svg
                            v-if="saving"
                            class="w-3.5 h-3.5 animate-spin"
                            fill="none"
                            viewBox="0 0 24 24"
                        >
                            <circle
                                class="opacity-25"
                                cx="12"
                                cy="12"
                                r="10"
                                stroke="currentColor"
                                stroke-width="4"
                            ></circle>
                            <path
                                class="opacity-75"
                                fill="currentColor"
                                d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"
                            ></path>
                        </svg>
                        {{
                            saving
                                ? "Menyimpan..."
                                : isEdit
                                  ? "Perbarui"
                                  : "Simpan"
                        }}
                    </button>
                </div>
            </div>
        </BaseModal>
    </div>
</template>
